...
1. Records with CAS that has matching SSD info were kept.
...
2. Dataset restricted to the chemicals (346) for which the SSDs were build on at least 3 species and with slope information (X10LogSSDSlope.ug.L..SigmaChronic.NOEC, X10LogSSDSlope.ug.L..SigmaChronic.NOEC)available. 
...
3. Records with unit information (ug/L and mg/L) were retained. 
...
4. Result units were all changed to ug/L.
...
5. Records with missing result mean value, maximum value, number of samples, LOQ value, number of samples below LOQ were excluded
...
6. Remove stations/entries do not reporting number of samples, keep only stations reporting more than 5 samples per year to report annual aggregates. 
...
7. Records with LOQ below 0 were excluded
...
data_prep <- rbind(data_prep,data.frame(Cat="Data Curation",Step=9,N.Sample=nrow(Data.AggBySiteID.2),N.CAS=length(unique(Data.AggBySiteID.2$CAS)),N.Country=length(unique(Data.AggBySiteID$countryCode))-1,N.Country=length(unique(Data.AggBySiteID.2$countryCode))-1,N.WaterBody=length(unique(Data.AggBySiteID.2$waterBodyIdentifier))-1,N.Site=length(unique(Data.AggBySiteID.2$monitoringSiteIdentifier)),N.SiteYear=length(unique(paste(Data.AggBySiteID.2$monitoringSiteIdentifier,Data.AggBySiteID.2$phenomenonTimeReferenceYear, sep=".")))))
...
if(interactive()) ggsave("inst/manuscript_2022/supp_fig1.png",width=5,height=3)
...
if(interactive()) ggsave("inst/manuscript_2022/supp_fig1_rmNA.png",width=5,height=3)
...
if(interactive()) ggsave("~/Projects/beemixtox/inst/manuscript_2022/supp_fig2.png",width=5,height=3)
```
